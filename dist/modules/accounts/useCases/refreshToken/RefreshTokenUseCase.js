...
let RefreshTokenUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(token) {
    ...
    if (!userToken) {
      throw new _AppError.AppError('Refresh Token does not exists!', 404);
    }
    ...
    const expiresDate = this.dayJSDateProvider.addDays(_auth.default.expires_refresh_token_days);
