...
let UpdateUserAvatarUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findById(user_id); // Verifica se já existe um Avatar inserido para o usuário
    ...
    ;
    await this.storageProvider.save(avatar_file, "avatar"); // Substituição do Avatar (caso já exista), ou inserção no caso de Novo
